...
- Every `*-check` adapter workflow generated by `src/main/kotlin/workflows/adapters/check/GradleCheck.kt` contains a job named `sha-pin-check` that calls `sha-pinning-guard.yml@v4`.
- Adapters affected: `app-check.yml`, `gradle-check.yml`, `gradle-plugin-check.yml`, `kotlin-library-check.yml`.
...
- The string is reported by GitHub as `<parent-job> / <called-reusable-workflow-job>` — currently:
  - `check / sha-pin-check / SHA Pinning Guard` (exact string verified empirically via Spec 10 pilot run).
...
2. Set `required_checks = ["check", "<Layer-3-status-check-string>"]` if you want Layer 3 enforcement.
...
- **Layer 1:** revert the `sha_pinning_required = true` attribute in Terraform; `make apply`. 15 resources flip back to `false` in one operation.
